...
public class TXServerPacketHandler {
  ...
  private void handleTypeChangePacket(ByteBufInputStream dat,
    ...
    try {
      ...
      TileEntityAutoSorter switcher =
          (TileEntityAutoSorter)world.getTileEntity(x, y, z);
      SortingInventory inv = switcher.chestSorts.get(
          MutablePair.of(new ChunkCoordinates(x2, y2, z2), side));
      inv.type = type;
      switcher.chestSorts.put(
          MutablePair.of(new ChunkCoordinates(x2, y2, z2), side), inv);
    } catch (Exception e) {
    }
  }
  ...
  public void onServerPacket(ServerCustomPacketEvent event) {
    ...
    try {
      ...
      if (packetID == 2 && world != null) {
        ...
        if (world.getEntityByID(readInt) != null) {
          EntityLivingBase target =
              (EntityLivingBase)world.getEntityByID(readInt);
          ...
          if (world.getEntityByID(readInt) != null) {
            ...
            if (player.getCurrentArmor(3) != null) {
              ...
              // player.worldObj.spawnParticle("explode", (double)(target.posX +
              // Math.random()-0.5F), (double)(target.boundingBox.maxY +
              // Math.random()/2), (double)(target.posZ + Math.random()-0.5F),
              // 0.0D, 0.0D, 0.0D);

              target.attackEntityFrom(DamageSourceTX.witherPlayerDamage(player),
                                      1);
      ...
      if (packetID == 6) {
        ...
        // ThaumicExploration.proxy.spawnEssentiaAtLocation(world, x+0.5F,
        // y+1.1F, z+0.5F, x2+0.5F, y2+0.5F, z2+0.5F, 5,color);
      }
      ...
      if (packetID == 1 && world != null) {
        ...
        if (type == 1) {
          ...
          world.setBlock(x, y, z, ThaumicExploration.boundChest,
          ...
          world.setBlock(x, y, z, ThaumicExploration.boundChest,
                         world.getBlockMetadata(x, y, z), 1);
          int nextID =
              player.inventory.getCurrentItem().stackTagCompound.getInteger(
                  "ID");
          ((TileEntityBoundChest)world.getTileEntity(x, y, z)).id = nextID;
          ((TileEntityBoundChest)world.getTileEntity(x, y, z))
              .setColor(15 - player.inventory.getCurrentItem().getItemDamage());
          ...
          int color = ((TileEntityBoundChest)world.getTileEntity(x, y, z))
                          .getSealColor();
          if (15 - (player.inventory.getCurrentItem().getItemDamage()) ==
              color) {

            int nextID =
                ((TileEntityBoundChest)world.getTileEntity(x, y, z)).id;
            ItemStack linkedSeal = new ItemStack(
                ThaumicExploration.chestSealLinked, 1,
          ...
          if (player.inventory.getCurrentItem().getItem() instanceof
            ...
            ((IFluidContainerItem)player.inventory.getCurrentItem().getItem())
                .fill(player.inventory.getCurrentItem(),
                      new FluidStack(FluidRegistry.WATER, 1000), true);
          } else if (player.inventory.getCurrentItem().getItem() ==
                     Items.bucket) {
