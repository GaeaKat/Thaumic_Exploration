...
public class SoulBrazierQueue {
  ...
  public void onConnected(PlayerLoggedInEvent event) {
    ...
    if (queueSize > 0) {
      ...
      for (int i = 0; i < queueSize; i++) {
        SoulBrazierQueueData aQueueData =
        ...
        if (SoulBrazierUtils.removePlayerDataFromWarpQueue(
                aPlayerUUID, aQueueData.aTileX, aQueueData.aTileY,
                aQueueData.aTileZ, aQueueData.aDimension)) {
          updateSoulBrazier(getWorldFromID(aQueueData.aDimension),
                            aQueueData.aTileX, aQueueData.aTileY,
                            aQueueData.aTileZ);
        }
      }
    }
  ...
  private static boolean updateSoulBrazier(World aWorld, int aX, int aY,
                                           int aZ) {
