...
public Item getItemDropped(int par1, Random par2Random, int par3)
{
  ...
  public void breakBlock(World par1World, int par2, int par3, int par4,
                         Block par5, int par6) {
    TileEntityBoundJar te =
        (TileEntityBoundJar)par1World.getTileEntity(par2, par3, par4);

    ItemStack drop = new ItemStack(ThaumicExploration.blankSeal, 1,
                                   ((TileEntityBoundJar)te).getSealColor());
  ...
  public ArrayList<ItemStack> getDrops(World world, int x, int y, int z,
    ...
    /*  ArrayList<ItemStack> drops = new ArrayList();
      ...
      (((TileJarFillable)te).aspectFilter == null)) { drop = new
      ItemStack(ConfigBlocks.blockJar);
          }
          if (((TileJarFillable)te).amount > 0) {
            ((ItemJarFilled)drop.getItem()).setAspects(drop, new
      AspectList().add(((TileJarFillable)te).aspect,
      ((TileJarFillable)te).amount));
          }
    ...
    ArrayList<ItemStack> drops = new ArrayList();
    /*
    drops.add(new ItemStack(ConfigBlocks.blockJar)); //TODO this is somehow
    causing 2 jars to drop return drops;*/
  ...
  public boolean onBlockActivated(World world, int x, int y, int z,
                                  EntityPlayer player, int par6, float par7,
