...
public class BlockBoundChest extends BlockContainer {
  ...
  public void onBlockPlacedBy(World par1World, int par2, int par3, int par4,
    ...
    int l1 =
        MathHelper.floor_double(
            (double)(par5EntityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) &
        3;
        ...
        : (par1World.getBlock(par2 - 1, par3, par4) == this
               ...
               : (par1World.getBlock(par2 + 1, par3, par4) == this
                      ...
                      : (par1World.getBlock(par2, par3, par4 - 1) == this
                             ? true
                             : par1World.getBlock(par2, par3, par4 + 1) ==
                                   this)));
  }
  ...
  public void dropItem(ItemStack itemstack, World par1World, int par2, int par3,
                       int par4) {
    if (itemstack != null) {
      ...
      for (float f2 = this.random.nextFloat() * 0.8F + 0.1F;
           itemstack.stackSize > 0; par1World.spawnEntityInWorld(entityitem)) {
        ...
        entityitem = new EntityItem(
            par1World, (double)((float)par2 + f), (double)((float)par3 + f1),
   ...
   * different block ID, or the same block with a different metadata value, but
   * before the new metadata value is set. Args: World, x, y, z, old block ID,
   * old metadata
   */
  public void breakBlock(World par1World, int par2, int par3, int par4,
    ...
    if (tileentitychest != null) {
      this.dropItem(
          new ItemStack(ThaumicExploration.blankSeal, 1,
                        15 - ((TileEntityBoundChest)par1World.getTileEntity(
                                  par2, par3, par4))
                                 .getSealColor()),
   ...
   */
  public IInventory getInventory(World par1World, int par2, int par3,
                                 int par4) {
    Object object =
        (TileEntityBoundChest)par1World.getTileEntity(par2, par3, par4);
   ...
   * based on its state.
   */
  public boolean canProvidePower() { return this.chestType == 1; }
  ...
  public int isProvidingWeakPower(IBlockAccess par1IBlockAccess, int par2,
    ...
    if (!this.canProvidePower()) {
      ...
      int i1 = ((TileEntityBoundChest)par1IBlockAccess.getTileEntity(par2, par3,
                                                                     par4))
                   .numUsingPlayers;
  ...
  public int isProvidingStrongPower(IBlockAccess par1IBlockAccess, int par2,
                                    int par3, int par4, int par5) {
    return par5 == 1
        ? this.isProvidingWeakPower(par1IBlockAccess, par2, par3, par4, par5)
        : 0;
  }
        ...
        par0World
            .getEntitiesWithinAABB(EntityOcelot.class,
                                   AxisAlignedBB.getBoundingBox(
                                       (double)par1, (double)(par2 + 1),
                                       (double)par3, (double)(par1 + 1),
                                       (double)(par2 + 2), (double)(par3 + 1)))
            .iterator();
  ...
  public int getComparatorInputOverride(World par1World, int par2, int par3,
    ...
    return Container.calcRedstoneFromInventory(
        this.getInventory(par1World, par2, par3, par4));
  }
   ...
   */
  public void
  registerBlockIcons(IIconRegister par1IconRegister) {
