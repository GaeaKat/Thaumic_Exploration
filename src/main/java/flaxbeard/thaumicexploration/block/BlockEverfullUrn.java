...
public class BlockEverfullUrn extends BlockContainer {
  ...
  public void randomDisplayTick(World par1World, int par2, int par3, int par4,
    ...
    // (double)(f2 + f3), 0.0D, 0.0D, 0.0D); par1World.spawnParticle("flame",
    // (double)(f + f4), (double)f1, (double)(f2 + f3), 0.0D, 0.0D, 0.0D);
  }
    ...
    if (activeStack != null) {
      if (activeStack.getItem() == Items.bucket) {
        ...
        ItemStack filledStack = FluidContainerRegistry.fillFluidContainer(
            water, activeStack.splitStack(1));
        if (filledStack != null) {
          world.playSoundAtEntity(entityPlayer, "liquid.water", 0.5F, 1.0F);
        ...
        if (((IFluidContainerItem)filledStack.getItem())
                .fill(filledStack, water, true) > 0) {
          world.playSoundAtEntity(entityPlayer, "liquid.water", 0.5F, 1.0F);
          if (!entityPlayer.inventory.addItemStackToInventory(filledStack)) {
            entityPlayer.dropPlayerItemWithRandomChoice(filledStack, false);
          }
