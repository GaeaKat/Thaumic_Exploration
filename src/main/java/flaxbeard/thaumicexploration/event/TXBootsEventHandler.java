...
public class TXBootsEventHandler {
  ...
  public static final PlayerCapabilities genericPlayerCapabilities =
...
ThaumicExploration.fakeAspectNecro)) {
//
System.out.println(event.entity.worldObj.isRemote+" has discovered fake
aspect");
//			}
//		}
    }*/
    if ((event.entity instanceof EntityPlayer)) {
      ...
      // Utils.getWalkSpeed(genericPlayerCapabilities)); updateSpeed(player);
      checkAir(player);
      ...
      if (player.getHeldItem() != null && player.getHeldItem().getItem()
        ...
        if (wand.getObjectInUse(player.getHeldItem(), player.worldObj) !=
                null &&
            wand.getCap(player.getHeldItem()) ==
                ThaumicExploration.WAND_CAP_MECHANIST) {
          if (wand.getObjectInUse(player.getHeldItem(), player.worldObj)
                  instanceof INode) {
            ...
            int nodeTicks =
                player.getHeldItem().stackTagCompound.getInteger("nodeTicks");
            nodeTicks++;

            TileNode node = (TileNode)wand.getObjectInUse(player.getHeldItem(),
                                                          player.worldObj);
            node.onUsingWandTick(player.getHeldItem(), player, nodeTicks);
            if (nodeTicks % 3 == 0) {
              if (player.worldObj.rand.nextBoolean()) {
                player.worldObj.playSound(
                    node.xCoord, node.yCoord, node.zCoord, "tile.piston.in",
                    0.1F + (float)(0.5F * Math.random()), 0.75F, false);
              } else {
                player.worldObj.playSound(
                    node.xCoord, node.yCoord, node.zCoord, "tile.piston.out",
                    0.1F + (float)(0.5F * Math.random()), 0.75F, false);
              }
            }
            player.getHeldItem().stackTagCompound.setInteger("nodeTicks",
                                                             nodeTicks);
          }
        }
        if (wand.getCap(player.getHeldItem()) ==
                ThaumicExploration.WAND_CAP_SOJOURNER &&
          ...
          if (nodeTicks >= 20) {
              ...
              for (int yy = -8; yy <= 8; yy++) {
                for (int zz = -8; zz <= 8; zz++) {
                  ...
                  if ((te instanceof INode) && emptyAspects.size() > 0) {
                    ...
                    if (canAdd)
                      nodes.add(new ChunkCoordinates(((int)player.posX) + xx,
                                                     ((int)player.posY) + yy,
                                                     ((int)player.posZ) + zz));
                  }
                }
            ...
            if (nodes.size() != 0) {
              if (!nodes.contains(new ChunkCoordinates(
                      player.getHeldItem().stackTagCompound.getInteger(
                          "drainX"),
                      player.getHeldItem().stackTagCompound.getInteger(
                          "drainY"),
                      player.getHeldItem().stackTagCompound.getInteger(
                          "drainZ")))) {
                randNode =
                    nodes.get(player.worldObj.rand.nextInt(nodes.size()));
                player.getHeldItem().stackTagCompound.setInteger("drainX",
                                                                 randNode.posX);
                player.getHeldItem().stackTagCompound.setInteger("drainY",
                                                                 randNode.posY);
                player.getHeldItem().stackTagCompound.setInteger("drainZ",
                                                                 randNode.posZ);
              } else {
              ...
              INode node = (INode)player.worldObj.getTileEntity(
                  randNode.posX, randNode.posY, randNode.posZ);
              ...
              Aspect takeAspect =
                  possibleAspects.getAspects()[player.worldObj.rand.nextInt(
                      possibleAspects.getAspects().length)];
              node.takeFromContainer(takeAspect, 1);
              player.worldObj.markBlockForUpdate(randNode.posX, randNode.posY,
                                                 randNode.posZ);
      ...
      if (!wasTainted && isTainted) {
        player.worldObj.playSoundEffect(player.posX, player.posY, player.posZ,
                                        "thaumcraft:zap", 1.0F, 1.0F);
        //        	player.addPotionEffect(new
        //        PotionEffect(Potion.blindness.id,60));
        //        	player.addPotionEffect(new
        //        PotionEffect(Potion.confusion.id,160));
      }
      player.getEntityData().setBoolean("tainted", isTainted);

      if (!(player.worldObj.getBiomeGenForCoords((int)player.posX,
                                                 (int)player.posZ) ==
        ...
        if (isTainted) {
          ...
          if (player.getActivePotionEffect(
                  ThaumicExploration.potionTaintWithdrawl) == null &&
              taintGP > 100) {
            for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
              if (player.inventory.getStackInSlot(i) != null &&
                  player.inventory.getStackInSlot(i).getItem() ==
            ...
            if (taintGP > 100)
              player.addPotionEffect(new PotionEffect(
                  ThaumicExploration.potionTaintWithdrawl.id, 100, 1));
          }
        }
          ...
          if (isTainted) {
            ...
            if (player.getFoodStats().getFoodLevel() < 4) {
              ...
              if (taint > 80) {
                ...
                taint = 0;
                if (player.worldObj.isRemote) {
                  Thaumcraft.proxy.swarmParticleFX(player.worldObj, player,
                                                   0.1F, 10.0F, 0.0F);
                }
              }
    ...
    if ((event.entity.worldObj.isRemote) &&
        (this.prevStep.containsKey(
            Integer.valueOf(event.entity.getEntityId()))) &&
      ...
      event.entity.stepHeight = ((Float)this.prevStep.get(Integer.valueOf(
  ...
  @SubscribeEvent
        public void onEntityUpdate(LivingUpdateEvent event){
          if (event.entity instanceof EntityPlayer) {
                ...
                PotionEffect effect =
  player.getActivePotionEffect(Config.potionTaintPoisonID); if (!(effect ==
  null)) {

                                PotionEffect neweffect = new
  PotionEffect(Potion.nightVision.id, Integer.MAX_VALUE, -42, true);
                        ...
                        /*if(player.isPotionActive(PotionFluxTaint.instance.id)){
                                ...
                                for (int i = 0;
  i<player.inventory.getSizeInventory(); i++) { if
  ...
  public void checkAir(EntityPlayer player) {
    ...
    if ((player.inventory.armorItemInSlot(0) != null) &&
      ...
      if (player.onGround || player.isOnLadder()) {
        ...
        ticksAir = 0;
        if (size > 0) {
          player.worldObj.createExplosion(player, player.posX, player.posY,
                                          player.posZ, size, false);
        }
      }
      ...
      if (smashing) {
        ...
        player.worldObj.spawnParticle(
            "flame", (double)(player.posX + Math.random() - 0.5F),
            (double)(player.posY + Math.random() - 0.5F),
            (double)(player.posZ + Math.random() - 0.5F), 0.0D, 0.0D, 0.0D);
        player.worldObj.spawnParticle(
            "smoke", (double)(player.posX + Math.random() - 0.5F),
            (double)(player.posY + Math.random() - 0.5F),
            (double)(player.posZ + Math.random() - 0.5F), 0.0D, 0.0D, 0.0D);
        player.worldObj.spawnParticle(
            "flame", (double)(player.posX + Math.random() - 0.5F),
        ...
        double motion = Math.abs(player.motionX) + Math.abs(player.motionZ) +
                        Math.abs(0.5 * player.motionY);
        if (!player.isWet() && motion > 0.1F) {
          player.worldObj.spawnParticle(
              "flame", (double)(player.posX + Math.random() - 0.5F),
              (double)(player.boundingBox.minY + 0.25F +
                       ((Math.random() - 0.5) * 0.25F)),
              (double)(player.posZ + Math.random() - 0.5F), 0.0D, 0.025D, 0.0D);
          // player.worldObj.spawnParticle("flame", (double)(player.posX +
          // Math.random()-0.5F), (double)(player.boundingBox.minY + 0.25F +
          // ((Math.random()-0.5)*0.25F)), (double)(player.posZ +
    ...
    // comet boots
          ...
          for (int z = -5; z < 6; z++) {
            if ((player.worldObj.getBlock(
                 ...
                 player.worldObj.getBlock(
                     ...
                     (int)(player.posZ + z)) == Blocks.water) &&
                player.worldObj
                        .getBlock((int)(player.posX + x), (int)player.posY - 1,
                                  (int)(player.posZ + z))
                        .getMaterial() == Material.water &&
                player.worldObj.getBlockMetadata((int)(player.posX + x),
                                                 (int)player.posY - 1,
                                                 (int)(player.posZ + z)) == 0 &&
                !player.isInWater() && (Math.abs(x) + Math.abs(z) < 8)) {
              player.worldObj.setBlock(
                  (int)(player.posX + x), (int)player.posY - 1,
                  (int)(player.posZ + z), ThaumicExploration.meltyIce);
              player.worldObj.spawnParticle(
                  "snowballpoof", (int)(player.posX + x), (int)player.posY,
                  (int)(player.posZ + z), 0.0D, 0.025D, 0.0D);
            }
          }
        }
      }
      ...
      if (!player.isWet() && motion > 0.1F) {
        player.worldObj.spawnParticle(
            "fireworksSpark", (double)(player.posX + Math.random() - 0.5F),
            (double)(player.boundingBox.minY + 0.25F +
                     ((Math.random() - 0.5) * 0.25F)),
            (double)(player.posZ + Math.random() - 0.5F), 0.0D, 0.025D, 0.0D);
