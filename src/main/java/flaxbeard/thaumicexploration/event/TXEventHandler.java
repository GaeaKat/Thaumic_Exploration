...
public class TXEventHandler {
  private HashMap<String, ArrayList<EntityItem>> lastKilled =
      new HashMap<String, ArrayList<EntityItem>>();
  ...
  @SideOnly(Side.CLIENT)
  public void renderMarkedBlocks(RenderWorldLastEvent event, float partialTicks,
                                 EntityPlayer player, long time) {
    Minecraft mc = Minecraft.getMinecraft();
    if ((player.inventory.getCurrentItem().hasTagCompound()) &&
        (!player.inventory.getCurrentItem().stackTagCompound.hasKey(
            "golemid"))) {
      Entity golem = null;
      ...
      if ((player.inventory.getCurrentItem().getItem() instanceof
        ...
        cc = new ChunkCoordinates(item.stackTagCompound.getInteger("brainx"),
                                  item.stackTagCompound.getInteger("brainy"),
                                  item.stackTagCompound.getInteger("brainz"));

        // cc =
        // ItemGolemBell.getGolemHomeCoords(player.inventory.getCurrentItem());
        // face =
        // ItemGolemBell.getGolemHomeFace(player.inventory.getCurrentItem());

        // int gid =
        // ItemGolemBell.getGolemId(player.inventory.getCurrentItem());
        // if (gid > -1) {
        ...
        if (player.worldObj.getTileEntity(
            ...
            !(player.worldObj.getTileEntity(
                  ...
                  item.stackTagCompound.getInteger("brainy"),
                  item.stackTagCompound.getInteger("brainz"))
                  instanceof TileEntityAutoSorter)) {
          return;
        }
      }
      GL11.glPushMatrix();
      ...
      if ((cc != null) &&
          (player.getDistanceSq(cc.posX, cc.posY, cc.posZ) < 4096.0D)) {
        ...
        for (int i = 0; i < 6; i++) {
          ...
          drawGolemHomeOverlay(
              ...
              cc.posZ + ForgeDirection.getOrientation(i).offsetZ, i,
              partialTicks);
          GL11.glPopMatrix();
        }
      }
      ...
      TileEntityAutoSorter sorter =
          (TileEntityAutoSorter)player.worldObj.getTileEntity(
              item.stackTagCompound.getInteger("brainx"),
              item.stackTagCompound.getInteger("brainy"),
              item.stackTagCompound.getInteger("brainz"));
      if (sorter != null) {
        for (MutablePair chest : sorter.chests) {
          ...
          if (player.getDistanceSq(x, y, z) < 4096.0D) {
            ...
            if (player.worldObj.isAirBlock(ox, oy, oz)) {
              GL11.glPushMatrix();
              // for (int a = 0; a < 6; a++) {
              drawAirBlockoverlay(ox + ForgeDirection.getOrientation(s).offsetX,
  ...
  public void drawMarkerOverlay(double x, double y, double z, int side,
    ...
    GL11.glTranslated(-iPX + x + 0.5D + dir.offsetX * 0.01F,
    ...
    UtilsFX.renderQuadCenteredFromTexture("textures/misc/mark.png", 1.0F, r, g,
                                          b, 200, 1, 1.0F);
    GL11.glPopMatrix();
  ...
  public void drawMarkerLine(double x, double y, double z, int side,
                             float partialTicks, int color,
                             TileEntityAutoSorter sorter) {
    EntityPlayer player =
        (EntityPlayer)Minecraft.getMinecraft().renderViewEntity;
    ...
    for (int i = 0; i <= length; i++) {
      ...
      double dx =
          dc1x +
          MathHelper.sin((float)((side * 20 + z % 16.0D +
      ...
      double dy =
          dc1y +
          MathHelper.sin((float)((side * 20 + x % 16.0D +
                                  dist * (1.0F - f2) * Config.golemLinkQuality -
                                  time % 32767.0F / 5.0F) /
                                 3.0D)) *
              0.5F * f3;
      double dz =
          dc1z +
          MathHelper.sin((float)((side * 20 + y % 16.0D +
                                  dist * (1.0F - f2) * Config.golemLinkQuality -
                                  time % 32767.0F / 5.0F) /
                                 2.0D)) *
              0.5F * f3;
  ...
  public void drawAirBlockoverlay(double x, double y, double z, int side,
                                  float partialTicks, int color) {
    float r = 1.0F;
    float g = 1.0F;
    float b = 1.0F;
    ...
    UtilsFX.renderQuadCenteredFromTexture("textures/blocks/empty.png", 1.0F, r,
                                          g, b, 200, 1, 1.0F);
    GL11.glPopMatrix();
  ...
  public void drawGolemHomeOverlay(double x, double y, double z, int side,
                                   float partialTicks) {
    float r = 1.0F;
    float g = 1.0F;
    float b = 1.0F;
    ...
    UtilsFX.renderQuadCenteredFromTexture("textures/misc/home.png", 1.0F, r, g,
                                          b, 200, 1, 1.0F);
    GL11.glPopMatrix();
  ...
  public void handleTaintSpawns(EntityJoinWorldEvent event) {
    if (event.entity instanceof EntityPlayer) {
      ...
      //			rp.addDiscoveredAspect(((EntityPlayer)event.entity).username,
      //ThaumicExploration.fakeAspectNecro); 			if
      //(rp.hasDiscoveredAspect(((EntityPlayer)event.entity).username,
      //ThaumicExploration.fakeAspectNecro)) {
      //				System.out.println(event.entity.worldObj.isRemote+"
      //has discovered fake aspect");
      //			}
    }
    if (event.entity instanceof ITaintedMob) {
      EntityLiving mob = (EntityLiving)event.entity;
      List<EntityAITaskEntry> tasksToRemove =
          new ArrayList<EntityAITaskEntry>();
      ...
      mob.targetTasks.addTask(
          1, new EntityAINearestAttackablePureTarget(
                 (EntityCreature)mob, EntityPlayer.class, 0, true));
    }
  }
  ...
  public void stopCreeperExplosions(LivingUpdateEvent event) {
    if (!isEnderIO && ConfigTX.enchantmentNVEnable &&
        event.entityLiving.getEquipmentInSlot(4) != null) {
      ItemStack heldItem = event.entityLiving.getEquipmentInSlot(4);
      int nightVision = EnchantmentHelper.getEnchantmentLevel(
          ThaumicExploration.enchantmentNightVision.effectId, heldItem);
      if (nightVision > 0 &&
          (!event.entityLiving.isPotionActive(Potion.nightVision.id) ||
           event.entityLiving.getActivePotionEffect(Potion.nightVision)
                   .getDuration() < 210)) {
        event.entityLiving.addPotionEffect(
            new PotionEffect(Potion.nightVision.id, 210, 1, true));
      }
    }
    if (event.entityLiving instanceof EntityCreeper &&
      ...
      for (int i = 0; i < size; i++) {
        EntityAITaskEntry entry =
            (EntityAITaskEntry)creeper.tasks.taskEntries.get(i);
      ...
      // ReflectionHelper.setPrivateValue(EntityCreeper.class, (EntityCreeper)
      // event.entityLiving, 2, LibObfuscation.TIME_SINCE_IGNITED);
  ...
  //!event.drops.contains(Item.itemsList[Block.tallGrass]) && event.block ==
  //Block.tallGrass && event.world.getBiomeGenForCoords(event.x, event.z) ==
  //ThaumcraftWorldGenerator.biomeTaint) { 			event.drops.clear();
  ...
  public void handleEnchantmentAttack(LivingAttackEvent event) {
    ...
    if ((event.entityLiving instanceof EntityEnderman ||
      ...
      int binding = EnchantmentHelper.getEnchantmentLevel(
          ThaumicExploration.enchantmentBinding.effectId, heldItem);
      if (binding > 1) {
        event.entityLiving.addPotionEffect(
            new PotionEffect(ThaumicExploration.potionBinding.id, 50, 1));
      }
    }
    if (event.source.getSourceOfDamage() instanceof EntityLivingBase &&
      ...
      if (binding > 0) {
        event.entityLiving.addPotionEffect(
            new PotionEffect(Potion.moveSlowdown.id, 50, 1));
      }
    }
    if (event.source.getSourceOfDamage() instanceof EntityLivingBase &&
        ConfigTX.enchantmentDisarmEnable) {
      EntityLivingBase attacker =
          (EntityLivingBase)event.source.getSourceOfDamage();
      ItemStack heldItem = attacker.getHeldItem();
      if (heldItem == null)
      ...
      int disarm = EnchantmentHelper.getEnchantmentLevel(
          ThaumicExploration.enchantmentDisarm.effectId, heldItem);
    ...
    if (event.source.getSourceOfDamage() instanceof EntityLivingBase) {
      ...
      if (heldItem.getItem() instanceof ItemWandCasting) {
        ...
        if (test == ThaumicExploration.STAFF_ROD_NECRO) {
          event.entityLiving.addPotionEffect(
              new PotionEffect(Potion.wither.id, 100));
        }
      }
    }
  }
  ...
  private ArrayList<ChunkCoordinates> safeBlocks(EntityPlayer player) {
    ...
    for (int x = -2; x < 3; x++) {
      for (int z = -2; z < 3; z++) {
        if (player.worldObj.isAirBlock((int)player.posX + x, (int)player.posY,
                                       (int)player.posZ + z)) {
          if (!player.worldObj.isAirBlock((int)player.posX + x,
                                          (int)player.posY - 1,
                                          (int)player.posZ + z)) {
            if (player.worldObj.isSideSolid(
                    (int)player.posX + x, (int)player.posY - 1,
                    (int)player.posZ + z, ForgeDirection.UP)) {
              safeSpots.add(new ChunkCoordinates((int)player.posX + x,
                                                 (int)player.posY,
                                                 (int)player.posZ + z));
            }
          }
  ...
  public void handleTaint(LivingHurtEvent event) {
    if (event.entityLiving instanceof EntityPlayer) {
      ...
      if (player.onGround && hasBauble(ThaumicExploration.tentacleRing,
        ...
        if (safeSpots.size() > 0) {
          ...
          EntityTaintacleMinion minion =
              new EntityTaintacleMinion(player.worldObj);
          minion.setPosition(safeSpots.get(rand).posX + 0.5F,
    ...
    if (event.entityLiving.worldObj.rand.nextInt(4) < 3) {
      ...
      if (event.source == DamageSourceThaumcraft.taint ||
          event.source == DamageSourceThaumcraft.tentacle ||
  ...
  public void handleItemUse(PlayerInteractEvent event) {
    ...
    if (event.entityPlayer.worldObj.blockExists(event.x, event.y, event.z)) {
      ...
      ConfigItems.itemGolemBell) { ItemStack stack
      =event.entityPlayer.getCurrentEquippedItem(); if
      (event.entityPlayer.worldObj.getBlock(event.x, event.y, event.z) ==
      ThaumicExploration.autoSorter) {
                              ...
                              if (stack.hasTagCompound())
                            {
                              ...
                              stack.getTagCompound().setInteger("brainx",
      event.x); stack.getTagCompound().setInteger("brainy", event.y);
                              stack.getTagCompound().setInteger("brainz",
      event.z); stack.getTagCompound().setInteger("brain", event.z);
                            }
                      ...
                      if (event.entityPlayer.worldObj.getTileEntity(event.x,
      event.y, event.z) != null && (!stack.hasTagCompound() ||
      !stack.getTagCompound().hasKey("golemid"))) { if
      (stack.getTagCompound().hasKey("brainx")) { ChunkCoordinates cc = new
      ChunkCoordinates(event.x,event.y,event.z); if
      (event.entityPlayer.worldObj.getTileEntity(event.x, event.y, event.z)
      instanceof IInventory) { TileEntityAutoSorter sorter =
      ...
      stack.getTagCompound().getInteger("brainz")); if
      ...
      (!event.entityPlayer.worldObj.isRemote)
                                              {
                                                  FMLNetworkHandler.openGui(event.entityPlayer,
      ThaumicExploration.instance, 2+event.face, event.entityPlayer.worldObj,
      ...
      // System.out.println(event.entityPlayer.worldObj.getBlock(event.x,
      // event.y, event.z) + " " + ThaumicExploration.boundChest);
      ...
      } else if (event.entityPlayer.worldObj.getBlock(event.x, event.y,
                                                      event.z) ==
                 ThaumicExploration.boundChest) {
        World world = event.entityPlayer.worldObj;
        // System.out.println(event.entityPlayer.worldObj.isRemote +
        // ItemBlankSeal.itemNames[((TileEntityBoundChest)
        // world.getBlockTileEntity(event.x, event.y,
        // event.z)).getSealColor()]);
        if (event.entityPlayer.inventory.getCurrentItem() != null) {
          if (event.entityPlayer.inventory.getCurrentItem().getItem() ==
              ThaumicExploration.chestSeal) {
            int color = ((TileEntityBoundChest)world.getTileEntity(
                             event.x, event.y, event.z))
                            .clientColor;
            type = 3;
            if (15 - (event.entityPlayer.inventory.getCurrentItem()
                          .getItemDamage()) ==
                color) {
              int nextID = ((TileEntityBoundChest)world.getTileEntity(
                                event.x, event.y, event.z))
                               .id;
              ItemStack linkedSeal =
                  new ItemStack(ThaumicExploration.chestSealLinked, 1,
    ...
    if (event.entityPlayer.worldObj.blockExists(event.x, event.y, event.z)) {
      if (event.entityPlayer.worldObj.getBlock(event.x, event.y, event.z) ==
          ...
          event.entityPlayer.worldObj.getBlockMetadata(event.x, event.y,
                                                       event.z) == 0) {
        if (event.entityPlayer.inventory.getCurrentItem() != null &&
            ((TileJarFillable)event.entityPlayer.worldObj.getTileEntity(
                 event.x, event.y, event.z))
                    .aspectFilter == null &&
            ((TileJarFillable)event.entityPlayer.worldObj.getTileEntity(
                 event.x, event.y, event.z))
  ...
  public void playerLogin(PlayerEvent.PlayerLoggedInEvent event) {
    for (Map.Entry<String, AspectList> entry :
         BoundJarNetworkManager.getData().networks.entrySet()) {
      PacketCustom.sendToPlayer(BoundJarNetworkManager.getPacket(new Tuple(
