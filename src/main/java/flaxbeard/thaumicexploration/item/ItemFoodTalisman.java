...
public class ItemFoodTalisman extends Item {
  ...
  public void addInformation(ItemStack par1ItemStack,
    ...
    par3List.add("Currently holds " +
                 (int)par1ItemStack.stackTagCompound.getFloat("food") +
                 " food points and " +
  ...
  public void onUpdate(ItemStack par1ItemStack, World par2World,
    ...
    if (par3Entity instanceof EntityPlayer && !par2World.isRemote &&
      ...
      for (int i = 0; i < 10; i++) {
        if (player.inventory.getStackInSlot(i) != null) {
          ...
          if (isEdible(food, player)) {
            ...
            if (par1ItemStack.stackTagCompound.getFloat("food") + (int)heal <
                100) {
              if (par1ItemStack.stackTagCompound.getFloat("saturation") + sat <=
                  100) {
                par1ItemStack.stackTagCompound.setFloat(
                    "saturation",
                    par1ItemStack.stackTagCompound.getFloat("saturation") +
                        sat);
              } else {
              ...
              par1ItemStack.stackTagCompound.setFloat(
                  "food",
                  par1ItemStack.stackTagCompound.getFloat("food") + (int)heal);
            }
          }
        }
      }
      if ((player.getFoodStats().getFoodLevel() < 20) &&
        ...
        if (Loader.isModLoaded("AppleCore")) {
          ...
          ObfuscationReflectionHelper.setPrivateValue(
              FoodStats.class, player.getFoodStats(),
      ...
      if ((player.getFoodStats().getSaturationLevel() <
        ...
        float finalSat = 0;
        if (player.getFoodStats().getFoodLevel() -
                player.getFoodStats().getSaturationLevel() <
            sat) {
          finalSat = sat - (player.getFoodStats().getFoodLevel() -
                            player.getFoodStats().getSaturationLevel());
        ...
        if (Loader.isModLoaded("AppleCore")) {
          ...
          ObfuscationReflectionHelper.setPrivateValue(
              FoodStats.class, player.getFoodStats(),
              (player.getFoodStats().getFoodLevel() + sat), "field_75125_b",
              "foodSaturationLevel");
      ...
      try {
        ...
        for (int i = 1; i < 25; i++) {
          EntityPlayer fakePlayer = new FakePlayerPotion(
              player.worldObj, new GameProfile(null, "foodTabletPlayer"));
          fakePlayer.setPosition(0.0F, 999.0F, 0.0F);
          ((ItemFood)food.getItem())
              .onEaten(food.copy(), player.worldObj, fakePlayer);
